...
# Firebase Firestore Setup (Replace with your credentials file)
cred = credentials.Certificate(r'C:\\Users\\mariy\\OneDrive\\Desktop\\Gouri\\StreetSecuritySystem\\streetsecuritysystem-firebase-adminsdk-fbsvc-d8de1576bb.json')  # Update with your Firestore JSON key
...
def log_event(event_type):
    ...
    db.collection("SecurityEvents").add(data)  # Adds event to Firestore collection
    print(f"📌 Logged to Firestore: {event_type} at {timestamp}")
...
def start_detection():
    ...
    motionless_logged = False  # Prevent multiple logs for the same event
    ...
    while cap.isOpened():
        ...
        if pose_results.pose_landmarks:
            ...
            if is_motionless(current_pose_landmarks, previous_pose_landmarks):
                ...
                elif time.time() - motionless_start_time > 5 and not motionless_logged:  # Check if 5 sec passed
                    print("🚶‍♂️ Human Detected Motionless!")
                    log_event("Motionless Human Detected")
                    motionless_logged = True  # Prevent duplicate logs
            else:
                motionless_start_time = None  # Reset timer if movement detected
                motionless_logged = False
            ...
            for hand_landmarks in hand_results.multi_hand_landmarks:
                ...
                if is_emergency_gesture(hand_landmarks.landmark):
                    ...
                    if not alarm_triggered:
                        ...
                        log_event("Emergency Gesture Detected")  # Log gesture detection
